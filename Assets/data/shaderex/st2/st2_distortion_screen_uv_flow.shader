Shader "streetball2/distortion/screen_uv_flow"
{
	Properties 
	{
		...
		_Factor ("Factor", Range(0, 1.0)) = 0.01
	...
	SubShader
	{
		...
		pass
		{
			...
			float4 frag (v2f i) : COLOR
			{
				...
				//half2 noiseOffset = half2(_SinTime.w, _CosTime.w) * _TimeFactor;
				half2 noiseOffset = _Time.yy * _TimeFactor;
				fixed2 noise = tex2D(_NoiseTex, i.uvMain.zw + noiseOffset).xy * 2 - 1;

				fixed2 offset = noise * tex2D(_MaskTex, i.uvMain.xy).r * _Factor;
