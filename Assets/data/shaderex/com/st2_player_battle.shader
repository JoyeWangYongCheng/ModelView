Shader "streetball2/model_battle" {
	Properties {
		...
		_LightMVP("MVP灯光方向",vector) = (0.3,0.5,0.8,0)
	...
	Subshader {
		...
		Pass {
			...
			CGPROGRAM
                ...
                #pragma multi_compile __ _MVP_PERSPECTIVE_
				...
				{	
                    ...
                    #ifdef _MVP_PERSPECTIVE_
                    float3 lightDir = normalize(_LightMVP).xyz;
                    //back to world
                    lightDir = mul(float4(lightDir,0),UNITY_MATRIX_V);
				...
				//	return ndl;
					...
					c = pow(c,1.2);
                    fixed4 final = lerp(c,c*(nrmAndMasks.b),_Light.w*-1) * _Color;
