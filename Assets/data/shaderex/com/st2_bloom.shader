Shader "streetball2/bloom"
{
	...
	float4 DownsampleFilter(sampler2D tex, float2 uv, float2 texelSize)
	{
	...
	float4 UpsampleFilter(sampler2D tex, float2 uv, float2 texelSize, float sampleScale)
	{
	...
	{
		float4 color = tex2D(_BaseTex, i.uvBase);
		float4 blur =  UpsampleFilter(_MainTex, i.uvMain, _MainTex_TexelSize.xy, 1);
		return SafeHDR((color + blur));
	}
//高斯
float4 fragBlurVertical(VaryingsDefault i):SV_Target
{
	...
	float3 sum = tex2D(_MainTex,uv).rgb*weight[2];
   ...
   return float4(sum*GAU,1.0);
}
float4 fragBlurHorizontal(VaryingsDefault i):SV_Target
{
	float weight[3]={0.4026,0.2442,0.0545};
...
//	return sum;	
 ...
 //  float3 sum01 = tex2D(_MainTex,uv01).rgb*weight[0];
	return float4(sum*GAU,1.0);
}


	float4 FragCombine(VaryingsMultitex i) : SV_Target
	{
		float4 base = (tex2D(_MainTex, i.uvMain));
		float4 blur = (tex2D(_BloomTex, i.uvMain));
		//float4 maskTex = tex2D(_MaskTex,i.uvMask);
		//return blur;
		return SafeHDR(base + base.a * blur * _Intensity);
	}
