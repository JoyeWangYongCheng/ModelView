 ...
 * 这里只是添加了几个常用的操作，其他操作需要的话可以自行添加。具体命令可参见TortoiseSVN的help功能，
...
{  
    ...
    private const string SVN_ADD = "Assets/SVN/Add &x";
    ...
    private const string SVNEXE= @"C:\Program Files\TortoiseSVN\bin\TortoiseProc.exe";
    private const string charbasetextures = "Assets/data/charbasetextures";
    private const string configmaterial = "Assets/data/configmaterial";
    private const string charadvancetextures = "Assets/data/texture/charadvancetextures";
    private const string stringArray = charbasetextures + "*" + configmaterial + "*" + charadvancetextures;
    /// <summary>  
    /// 创建一个SVN的cmd命令  
    /// </summary>  
	...
	{  
        ...
        Debug.Log(c);
		ProcessStartInfo info = new ProcessStartInfo(SVNEXE, c);  
    ...
    [MenuItem(SVN_ADD)]
    public static void SVNAdd()
    {

        SVNCommand(ADD, AddCommitSelectedObjectPath());
	...
	{  
        ...
        SVNCommand(COMMIT, AddCommitSelectedObjectPath());
    ...
    private static string AddCommitSelectedObjectPath()
    {
        string path = AssetsPathToFilePath(charbasetextures);
        //路径分隔符  
        path += "*";
        //meta文件  
        path += AssetsPathToFilePath(configmaterial) ;
        //路径分隔符  
        path += "*";
        path += AssetsPathToFilePath(charadvancetextures);
        path += "*";
        return path;
    }
    /// <summary>  
